...
BufferD3D11::initDeviceMemory()
{
    deviceMemories_ = reinterpret_cast<DeviceMemory*>(
        reinterpret_cast<char*>(this) + sizeof(BufferD3D11));
...
{
    deviceMemories_ = reinterpret_cast<DeviceMemory*>(
        reinterpret_cast<char*>(this) + sizeof(Image1DD3D11));
    memset(deviceMemories_, 0,
        context_().devices().size() * sizeof(DeviceMemory));
}
...
{
    deviceMemories_ = reinterpret_cast<DeviceMemory*>(
        reinterpret_cast<char*>(this) + sizeof(Image2DD3D11));
    memset(deviceMemories_, 0,
        context_().devices().size() * sizeof(DeviceMemory));
}
...
Image2DD3D11::mapExtObjectInCQThread()
{
    ...
    if(getUsage() == D3D11_USAGE_STAGING) {
	    ...
	    reinterpret_cast<ID3D11Texture2D*>(getD3D11Resource())
	        ->GetDesc(&texDesc);
...
{
    deviceMemories_ = reinterpret_cast<DeviceMemory*>(
        reinterpret_cast<char*>(this) + sizeof(Image3DD3D11));
    memset(deviceMemories_, 0,
        context_().devices().size() * sizeof(DeviceMemory));
}
...
Image3DD3D11::mapExtObjectInCQThread()
{
    ...
    if(getUsage() == D3D11_USAGE_STAGING) {
	    ...
	    reinterpret_cast<ID3D11Texture3D*>(getD3D11Resource())
	        ->GetDesc(&texDesc);
	    texDesc.Usage = D3D11_USAGE_STAGING;
	    texDesc.MipLevels = 1;
	    texDesc.BindFlags = 0;
